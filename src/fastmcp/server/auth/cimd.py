...
class CIMDTrustPolicy(BaseModel):
    ...
    def is_trusted(self, client_id_url: str) -> bool:
        ...
        try:
            ...
            if any(
                self._domain_matches(domain, blocked)
            ...
            return any(
                self._domain_matches(domain, trusted)
                for trusted in self.trusted_domains
            )
        except Exception as e:
            logger.warning(
                "Error checking trust for client_id %s: %s", client_id_url, e
            )
            return False
    ...
    def is_blocked(self, client_id_url: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Error checking blocklist for client_id %s: %s", client_id_url, e
            )
            return False
